...
let state = {
    ...
    logsCurrentPage: 1,
    logsPerPage: 100,
    allProfessors: [], // [NOVO] Para guardar a lista de professores
    currentFilters: {
        search: '',
        category: '',
...
async function populateShareModal() {
    ...
    try {
        ...
        // Popula a lista de professores para adicionar (excluindo os que já são membros)
        ...
        // Popula a lista de membros atuais
        currentMembersList.innerHTML = '';
        currentMembers.forEach(member => {
            const li = document.createElement('li');
            li.className = 'flex justify-between items-center text-sm';
            li.textContent = member.full_name;
            if (member.isOwner) {
                const ownerTag = document.createElement('span');
                ownerTag.className = 'ml-2 text-xs bg-red-200 text-red-800 px-2 py-0.5 rounded-full';
                ownerTag.textContent = 'Dono';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('addProfessorToClassBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            alert('Professor adicionado à turma!');
            await populateShareModal(); // Atualiza a lista de membros no modal
        } catch (error) {
            alert('Erro ao partilhar a turma: ' + error.message);
        }
    });
